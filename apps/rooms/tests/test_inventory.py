...
from apps.rooms.factories import InventoryFactory
from apps.rooms.models import Inventory
...
class InventoryCreateTestCase(BaseInventoryTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.admin_user = UserFactory.create(is_staff=True, is_superuser=True)
        self.client.force_authenticate(self.admin_user)

    def test_inventory_create_correct(self):
    ...
    def test_inventory_create_without_name(self):
        del self.body["name"]
        response = self.client.post(self.url_list, data=self.body)
    ...
    def test_inventory_create_non_admin_user(self):
    ...
    def test_inventory_create_anonymous_user(self):
        self.client.logout()
        response = self.client.post(self.url_list, data=self.body)
...
class InventoryUpdateTestCase(BaseInventoryTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.admin_user = UserFactory.create(is_staff=True, is_superuser=True)
        self.client.force_authenticate(self.admin_user)

    def test_inventory_update_correct(self):
        self.body.update({"name": "table"})
        response = self.client.put(self.url_detail, data=self.body)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get("name"), "table")

    def test_inventory_update_without_name(self):
        del self.body["name"]
        response = self.client.put(self.url_detail, data=self.body)
    ...
    def test_inventory_partial_update_correct(self):
        self.body.update({"name": "table"})
    ...
    def test_inventory_partial_update_empty_name(self):
        self.body.update({"name": ""})
        response = self.client.patch(self.url_detail, data=self.body)
    ...
    def test_inventory_partial_update_without_name(self):
        del self.body["name"]
        response = self.client.patch(self.url_detail, data=self.body)
...
class InventoryDeleteTestCase(BaseInventoryTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.admin_user = UserFactory.create(is_staff=True, is_superuser=True)
        self.client.force_authenticate(self.admin_user)

    def test_inventory_delete_correct(self):
        response = self.client.delete(self.url_detail)

        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(len(Inventory.objects.all()), 4)
